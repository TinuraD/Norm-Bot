...
from ef_norm.modules import ALL_MODULES
...
PM_START_TEXT = """
හායි, මම [Norm](https://t.me/efnormbot). මම Group Manament Bot කෙනෙක්. මාව පාවිච්චි කරන විදිහ දැන විධාන 📌 උඩ click කරන්න. වැඩි විස්තර දැන ගන්න විස්තර 📃 උඩ කරන්න.
"""
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("ef_norm.modules." + module_name)
...
@run_async
def ef_norm_about_callback(update, context):
    query = update.callback_query
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(
                            text="කොන්දේසි", callback_data="aboutmanu_tac"
                        ),
                    ],
                    [
                        InlineKeyboardButton(
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendMessage(f"@{SUPPORT_CHAT}", "Norm was updated By EF Bots ✅")
        except Unauthorized:
            LOGGER.warning(
