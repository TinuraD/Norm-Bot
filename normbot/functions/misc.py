...
def paginate_modules(page_n: int, module_dict: Dict, prefix, chat=None) -> List:
    ...
    if len(pairs) > 5:
        pairs = pairs[modulo_page * 4:4 * (modulo_page + 1)] + [[EqInlineKeyboardButton("More Tools 🧰", callback_data="aboutmenu_more")]] + [
            (EqInlineKeyboardButton("Back", callback_data="{}_prev({})".format(prefix, modulo_page)),
                EqInlineKeyboardButton("Close", callback_data="aboutmenu_back"),
             EqInlineKeyboardButton("Next", callback_data="{}_next({})".format(prefix, modulo_page)))]


    else:
        pairs += [[EqInlineKeyboardButton("close", callback_data="aboutmenu_")]]
