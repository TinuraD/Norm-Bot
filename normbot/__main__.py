...
from normbot.modules.helper_funcs.alternate import typing_action
from normbot.modules.helper_funcs.chat_status import is_user_admin
from normbot.modules.helper_funcs.misc import paginate_modules
from normbot.modules.helper_funcs.readable_time import get_readable_time

PM_START_TEXT = """
හායි, මම [normbot](https://t.me/efnormbotbot). මම Group Manament Bot කෙනෙක්. මාව පාවිච්චි කරන විදිහ දැන විධාන 📌 උඩ click කරන්න. වැඩි විස්තර දැන ගන්න විස්තර 📃 උඩ කරන්න.
"""
...
def normbot_about_callback(update, context):
    ...
    if query.data == "aboutmanu_":
        query.message.edit_text(
            text=f"හායි, මම [normbot](https://t.me/efnormbotbot)"
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendMessage(f"@{SUPPORT_CHAT}", "normbot was updated By EF Bots ✅")
        except Unauthorized:
            LOGGER.warning(
